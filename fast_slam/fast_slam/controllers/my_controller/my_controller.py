...
def eval_sensor_model(measurements, particles):
    ...
    for particle in particles:
        weight = 1.0  # Start with an initial weight
        ...
        for i, lm_id in enumerate(ids):
            ...
            # Get or initialize the landmark in the particle's map
            landmark = particle['landmarks'][lm_id]
            if not landmark['observed']:
                # Initialize landmark position and mark as observed
                landmark['mu'] = [particle['x'] + dx, particle['y'] + dy]
                landmark['sigma'] = R_t
            ...
            H = np.eye(2)  # Observation model Jacobian
            Q = landmark['sigma'] + R_t  # Measurement covariance
            ...
            # Update landmark mean and covariance using the Kalman filter equations
            K = np.dot(landmark['sigma'], np.linalg.inv(Q))  # Kalman gain
            innovation = z_measured - z_pred
            landmark['mu'] += np.dot(K, innovation)
            landmark['sigma'] = np.dot(np.eye(2) - K, landmark['sigma'])
