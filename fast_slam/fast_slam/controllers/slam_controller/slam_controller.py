...
def eval_sensor_model(measurements, particles):
    ...
    # Measured landmark IDs and relative distances
    ids = measurements['id']
    ...
    for particle in particles:
        for i, lm_id in enumerate(ids):
            ...
            mu_prev = np.array(landmark['mu'])
            sigma_prev = landmark['sigma']
            dx = x_dists[i]
            dy = y_dists[i]
            z_measured = np.array([dx, dy])

            if landmark['observed']:
                # Predicted location of the landmark
                z_hat = mu_prev  # Simplified because C_t = I

                # Compute measurement covariance Q
                Q = sigma_prev + R_t

                # Compute Kalman gain K
                K = sigma_prev @ np.linalg.inv(Q)

                # Update the mean of the landmark
                innovation = z_measured - z_hat
                mu_updated = mu_prev + K @ innovation

                # Update the covariance of the landmark
                I = np.eye(len(sigma_prev))
                sigma_updated = (I - K) @ sigma_prev

                # Predict measurement for this landmark
                z_pred = np.array([
                    mu_updated[0] - particle['x'],
                    mu_updated[1] - particle['y']
                ])

                # Calculate weight using the Gaussian probability density
                likelihood = norm.pdf(np.linalg.norm(z_measured - z_pred), 0, np.sqrt(Q[0, 0]))
                particle['weight'] *= likelihood

                # Store updated values back in the particle
                landmark['mu'] = mu_updated
                landmark['sigma'] = sigma_updated

            else:
                # Initialize unobserved landmarks
                landmark['mu'] = z_measured + np.array([particle['x'], particle['y']])
                landmark['sigma'] = R_t
                landmark['observed'] = True

def resample_particles(particles):
    """
    Performs resampling using stochastic universal sampling (SUS).
    Returns a new set of resampled particles based on their weights.
    """
    new_particles = []
    ...
    pointers = [start + i * step for i in range(num_particles)]
    ...
    for pointer in pointers:
        ...
        new_particle = copy.deepcopy(particles[index])
