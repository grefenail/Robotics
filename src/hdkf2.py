...
rospy.init_node('vision_kalman_tracking', anonymous=True)
...
def main():
    ...
    try:
                    ...
                    if class_id < len(labels) and labels[class_id] == "person":
                        ...
                        if selected_person == i:
                            ...
                            if len(xarray) >= 5 and len(yarray) >= 5:
                                if xarray[0] - xarray[4] > 50 or yarray[0] - yarray[4] > 50:
                                    print("rebooted ==============================")
                                    xarray = np.array([])
                                    yarray = np.array([])
                                    xarray = np.append(xarray, [person_center_x])
                                ...
                                predicted=kf.predict(person_center_x, person_center_y)
                                # x = kf.x
                                # y = kf.y
                                print("predicted x is: " + str(predicted[0]))
                                print("predicted y is: " + str(predicted[1]))

                                next_coordinate_x = predicted[0]
                                next_coordinate_y = predicted[1]
        ...
        plt.scatter(xarray, yarray, color='blue', label='Original Coordinates')

        # Plot the predicted coordinate point in yellow
        plt.scatter(next_coordinate_x, next_coordinate_y, color='yellow', marker='o', label='Predicted Coordinate')
...
def move(depth_value, control_command_x, control_command_y):
    ...
    if control_command_x < frame_center_x - 50:
        ...
        #body["vth"] = 0.0000001
        twist.linear.x = 0; twist.linear.y = 0; twist.linear.z = 0
        twist.angular.x = 0; twist.angular.y = 0; twist.angular.z = 45; 
        pub_move.publish(twist)
        cat = "positive"
        # pub_rot.publish(cat)
        #print(cat)
